...
export class Charting {
    ...
    lineChart() {
        var self = this;
        // const scaleX = this.scaleX = d3.scaleBand()
        //     .rangeRound([0, this.innerWidth]).padding(0.1);

        // const scaleY = this.scaleY = d3.scaleLinear()
        //     .range([this.innerHeight, 0]).clamp(true).nice(); 
        const scaleX = this.scaleX = this.d3ScaleX(this.innerWidth);

        const scaleY = this.scaleY = this.d3ScaleY(this.innerHeight);        

        this.d3line = d3.line()//.curve(d3.curveBasis)
            .x(function (d) { 
    ...
    fillArea(dataCol) {
        ...
        d3.select('svg').select('g')
            ...
            .attr('d', this.area);
        if (this.defaultStyles.lineInArea) {
            d3.select('svg').select('g')
            .append('path')
            .attr('class', 'chart-line')
            .data([dataCol])
    ...
    drawColumn(dataCol) {
         this.scaleX.domain(dataCol.map((data) => { return data[this.X] }));
         this.scaleY.domain([0, d3.max(dataCol, (data) => { return data[this.Y] })]);
         d3.select('svg')
    ...
    render() {
        ...
        if(this.defaultStyles.lableAxis) {
            this.lableAxis();
        }
    }
